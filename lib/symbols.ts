/**
 * Asset identity across the two venues.
 *
 ...
 * lists and what Variational lists. New listings on either venue appear
 * without a code change.
 *
 * Only the cases where the two venues genuinely disagree on naming need an
 * override.
 */
...
/**
 * Avantis base -> Variational ticker, only where plain equality fails.
 *
 * The 1000x contracts are a denomination difference, not an asset difference.
 * Funding is a percentage of notional, so it is invariant to the contract
 * multiplier and the match is exact for our purposes.
 *
 * SPY maps to US500, which despite the index-sounding ticker is Variational's
 * listing of the SPDR S&P 500 ETF itself, marked around 742 like the ETF. Do
 * not be tempted by Variational's SPX: that is SPX6900, a memecoin.
 */
export const VARIATIONAL_OVERRIDES: Record<string, string> = {
  WTIU6: "CL",
  WTIM6: "CL",
  BRENTV6: "BZ",
  SPY: "US500",
 ...
 */
export const MATCH_CAVEATS: Record<string, string> = {
  ...
  BRENTV6: "Avantis prices a specific Brent futures expiry, Variational quotes generic Brent",
};

export function variationalTickerFor(avantisBase: string): string {
  return VARIATIONAL_OVERRIDES[avantisBase] ?? avantisBase;
}
